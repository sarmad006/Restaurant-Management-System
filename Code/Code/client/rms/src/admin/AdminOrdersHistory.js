...
import {Box,Typography,Card, Rating,Button,Modal} from '@mui/material'
...
const AdminOrdersHistory = () => {
    ...
    useEffect(()=>{
        Axios.get("http://localhost:3001/admin/orderhistory/get",{params :{id:id}}).then((response)=>{
    ...
    return (
     ...
     <Card style={{marginLeft:"2rem",marginTop:"1rem",width:"60rem"}}>
     {data.map(orders=>(
         <div>
   ...
   <Typography variant="body" style={{marginLeft:"7rem"}}>
       {orders.customer_id}
   </Typography>
   <Rating style={{marginLeft:"3rem"}} readOnly value={orders.rating} />
     </div>
